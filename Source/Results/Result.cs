...
namespace Zentient.Results
{
    ...
    public readonly struct Result : IResult
    {
        ...
        /// <param name="message">An optional success message.</param>
        public static IResult<T> Success<T>(T value, string? message = null) => Result<T>.Success(value, message);

        /// <summary>Creates a successful generic result with a "Created" status.</summary>
        /// <typeparam name="T">The type of the value.</typeparam>
        /// <param name="value">The value to encapsulate.</param>
        /// <param name="message">An optional success message.</param>
        public static IResult<T> Created<T>(T value, string? message = null) => Result<T>.Created(value, message);

        /// <summary>Creates a successful generic result with a "No Content" status.</summary>
        /// <typeparam name="T">The type of the value.</typeparam>
        /// <param name="message">An optional success message.</param>
        public static IResult<T> NoContent<T>(string? message = null) => Result<T>.NoContent(message);

        /// <summary>Creates a generic failure result from a single error.</summary>
        /// <typeparam name="T">The type of the value (will be default).</typeparam>
        /// <param name="error">The error information.</param>
        /// <param name="status">Optional custom status. Defaults to <see cref="ResultStatuses.BadRequest"/>.</param>
        public static IResult<T> Failure<T>(ErrorInfo error, IResultStatus? status = null) => Result<T>.Failure(default, error, status ?? ResultStatuses.BadRequest);

        /// <summary>Creates a generic failure result from a collection of errors.</summary>
        /// <typeparam name="T">The type of the value (will be default).</typeparam>
        /// <param name="errors">A collection of error information.</param>
        /// <param name="status">Optional custom status. Defaults to <see cref="ResultStatuses.BadRequest"/>.</param>
        public static IResult<T> Failure<T>(IEnumerable<ErrorInfo> errors, IResultStatus? status = null) => Result<T>.Failure(default, errors, status ?? ResultStatuses.BadRequest);

        /// <summary>Creates a generic failure result representing validation errors.</summary>
        /// <typeparam name="T">The type of the value (will be default).</typeparam>
        /// <param name="errors">A collection of validation errors.</param>
        public static IResult<T> Validation<T>(IEnumerable<ErrorInfo> errors) => Result<T>.Validation(errors);

        /// <summary>Creates an unauthorized generic failure result.</summary>
        /// <typeparam name="T">The type of the value (will be default).</typeparam>
        /// <param name="error">Optional specific error info.</param>
        public static IResult<T> Unauthorized<T>(ErrorInfo? error = null) => Result<T>.Failure(default, error ?? new ErrorInfo(ErrorCategory.Authentication, "Unauthorized", "Authentication required."), ResultStatuses.Unauthorized);

        /// <summary>Creates a forbidden generic failure result.</summary>
        /// <typeparam name="T">The type of the value (will be default).</typeparam>
        /// <param name="error">Optional specific error info.</param>
        public static IResult<T> Forbidden<T>(ErrorInfo? error = null) => Result<T>.Failure(default, error ?? new ErrorInfo(ErrorCategory.Authorization, "Forbidden", "You do not have permission to perform this action."), ResultStatuses.Forbidden);

        /// <summary>Creates a not found generic failure result.</summary>
        /// <typeparam name="T">The type of the value (will be default).</typeparam>
        /// <param name="error">Optional specific error info.</param>
        public static IResult<T> NotFound<T>(ErrorInfo? error = null) => Result<T>.Failure(default, error ?? new ErrorInfo(ErrorCategory.NotFound, "NotFound", "The requested resource was not found."), ResultStatuses.NotFound);

        /// <summary>Creates a conflict generic failure result.</summary>
        /// <typeparam name="T">The type of the value (will be default).</typeparam>
        /// <param name="error">Optional specific error info.</param>
        public static IResult<T> Conflict<T>(ErrorInfo? error = null) => Result<T>.Failure(default, error ?? new ErrorInfo(ErrorCategory.Conflict, "Conflict", "A conflict occurred with the current state of the resource."), ResultStatuses.Conflict);

        /// <summary>Creates an internal server error generic result.</summary>
        /// <typeparam name="T">The type of the value (will be default).</typeparam>
        /// <param name="error">Optional specific error info.</param>
        public static IResult<T> InternalError<T>(ErrorInfo? error = null) => Result<T>.Failure(default, error ?? new ErrorInfo(ErrorCategory.General, "InternalError", "An unexpected error occurred."), ResultStatuses.Error);
